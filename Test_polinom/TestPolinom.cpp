...
TEST(List, can_copy_no_empty_list)
{
	TList<int>l;
	l.InsFirst(5);
	ASSERT_NO_THROW(TList<int> copy(l));
...
TEST(List, can_add_an_elem_to_the_end_of_the_no_empty_list)
{
	TList<int> l;
	l.InsFirst(5);
	ASSERT_NO_THROW(l.InsLast(5));
}
TEST(List, adding_an_elem_to_the_end_of_the_list_size_by_one)
{
	TList<int> l;
	l.InsLast(5);
	int size = l.Get_Size();
...
TEST(List, cant_delete_an_elem_from_an_empty_list)
{
	TList<int> l;
	l.InsLast(5);
	l.DelFirst();
...
TEST(List, adding_the_opposite_sign_to_an_existing_monome_removes_the_given_monome_from_the_list)
{
	TPolinom p;
...
TEST(Polinom, can_multiply_an_empty_polinom_by_a_empty_polinom)
{
	TPolinom p1,p2, res;
	ASSERT_NO_THROW(res = p1 * p2);
	ASSERT_NO_THROW(p1 *= p2);
}
TEST(Polinom, can_multiply_a_no_empty_polinom_by_a_empty_polinom)
{
	TPolinom p1, p2, res;
	TMonom m;
	p1.InsMonom(m);
...
TEST(Polinom, can_multiply_an_emty_polinom_by_a_no_empty_polinom)
{
	TPolinom p1, p2, res;
	TMonom m;
	p2.InsMonom(m);
